...
const SUBTITLE_TEXT = 'Gebruik alle 4 dobbelstenen één keer en beide bewerkingen om tot het juiste antwoord te komen!';
...
//  KANSREKENEN MET DOBBELSTENEN - generateQuestions 


var PROB_TEXT_SIZE      = 12;           // ← tekstgrootte vraagtekst (pixels)
var PROB_TEXT_COLOR     = [0,0,0]; // ← kleur vraagtekst [R, G, B]
var PROB_TEXT_STYLE     = 'bold';       // ← 'bold' of 'normal'
var PROB_TEXT_ALIGN     = 'left';       // ← uitlijning: 'left', 'center' of 'right'
var PROB_TEXT_MARGIN_X  = 6;            // ← horizontale marge links (en rechts bij center)
var PROB_TEXT_LINE_H    = 18;           // ← regelafstand vraagtekst

var PROB_TEXT_OFFSET_Y  = 8;            // ← ruimte tussen dobbelstenen en tekst
                                        //   positief = lager, negatief = hoger

var PROB_ANS_TEXT_SIZE  = 16;           // ← tekstgrootte antwoordblokjes
var PROB_ANS_TEXT_COLOR = [0, 0, 0];    // ← kleur antwoordtekst [R, G, B]

var PROB_DECIMALS       = 3;            // ← aantal decimalen (2 of 3)

// ── Alle uitkomsten genereren ─────────────────────────────────

function allOutcomes(n) {
  if (n === 1) return [[1],[2],[3],[4],[5],[6]];
  let sub = allOutcomes(n - 1);
  let res = [];
  for (let s of sub) for (let d = 1; d <= 6; d++) res.push([...s, d]);
  return res;
}

// Afronden op PROB_DECIMALS decimalen, kommanotatie
function fmt(val) {
  return val.toFixed(PROB_DECIMALS).replace('.', ',');
}

// ── Vraagdefinities ───────────────────────────────────────────
// Elke generator geeft { vraag, antwoord (string), antwoordGetal (number),
//                        dobbelstenen (array getoonde ogen) } of null

function tryMakeQuestion() {
  // Altijd 2 dobbelstenen
  let n = 2;
  let outcomes = allOutcomes(n); // 36 uitkomsten
  let total = 36;
  let fmt2 = val => parseFloat(val.toFixed(PROB_DECIMALS));

  // Visuele dobbelstenen (decoratief)
  let showDice = [
    Math.floor(Math.random() * 6) + 1,
    Math.floor(Math.random() * 6) + 1
  ];

  // 9 vraagtypen: 0-2 som, 3-4 product, 5-6 verschil, 7-8 extra
  ...
  if (type === 0) {
    ...
    for (let o of outcomes) {
      let s = o[0] + o[1];
      somCounts[s] = (somCounts[s] || 0) + 1;
    }
    let candidates = Object.entries(somCounts)
      .map(([s, cnt]) => ({ s: Number(s), cnt }))
      .filter(({ cnt }) => cnt > 0 && cnt < total);
    if (!candidates.length) return null;
    let { s, cnt } = candidates[Math.floor(Math.random() * candidates.length)];
    let antwoord = fmt2(cnt / total);
    if (antwoord <= 0 || antwoord >= 1) return null;
    return {
      vraag: `Gooi 2 dobbelstenen.\nP(som = ${s}) = ?`,
      antwoordStr: fmt(antwoord), antwoord,
      dice: showDice, label: `som=${s}`
    };
  }
  ...
  if (type === 1) {
    ...
    let validX = [];
    for (let x = 2; x <= 11; x++) {
      let cnt = outcomes.filter(o => o[0]+o[1] > x).length;
      let p = cnt / total;
      if (p >= 0.05 && p <= 0.95) validX.push({ x, cnt });
    }
    if (!validX.length) return null;
    let { x, cnt } = validX[Math.floor(Math.random() * validX.length)];
    let antwoord = fmt2(cnt / total);
    return {
      vraag: `Gooi 2 dobbelstenen.\nP(som > ${x}) = ?`,
      antwoordStr: fmt(antwoord), antwoord,
      dice: showDice, label: `som>${x}`
    };
  }
  ...
  if (type === 3) {
    ...
    let candidates = Object.entries(prodCounts)
      .map(([p, cnt]) => ({ p: Number(p), cnt }))
      .filter(({ cnt }) => cnt > 0 && cnt < total);
    if (!candidates.length) return null;
    let { p, cnt } = candidates[Math.floor(Math.random() * candidates.length)];
    let antwoord = fmt2(cnt / total);
    if (antwoord <= 0 || antwoord >= 1) return null;
    return {
      vraag: `Gooi 2 dobbelstenen.\nP(product = ${p}) = ?`,
      antwoordStr: fmt(antwoord), antwoord,
      dice: showDice, label: `product=${p}`
    };
  }

  if (type === 4) {
    // P(product > x) — kies x zodat kans zinvol is
    let validX = [];
    for (let x = 1; x <= 30; x++) {
      let cnt = outcomes.filter(o => o[0]*o[1] > x).length;
      let p = cnt / total;
      if (p >= 0.10 && p <= 0.90) validX.push({ x, cnt });
    }
    if (!validX.length) return null;
    let { x, cnt } = validX[Math.floor(Math.random() * validX.length)];
    let antwoord = fmt2(cnt / total);
  ...
  if (type === 5) {
    // P(verschil = x)  verschil = |d1 - d2|
    let versCounts = {};
    for (let o of outcomes) {
      let v = Math.abs(o[0] - o[1]);
      versCounts[v] = (versCounts[v] || 0) + 1;
    }
    let candidates = Object.entries(versCounts)
      .map(([v, cnt]) => ({ v: Number(v), cnt }))
      .filter(({ cnt }) => cnt > 0 && cnt < total);
    if (!candidates.length) return null;
    let { v, cnt } = candidates[Math.floor(Math.random() * candidates.length)];
    let antwoord = fmt2(cnt / total);
    if (antwoord <= 0 || antwoord >= 1) return null;
    return {
      vraag: `Gooi 2 dobbelstenen.\nP(verschil = ${v}) = ?`,
      antwoordStr: fmt(antwoord), antwoord,
      dice: showDice, label: `verschil=${v}`
    };
  }
  ...
  if (type === 6) {
    ...
    let validX = [];
    for (let x = 0; x <= 4; x++) {
      let cnt = outcomes.filter(o => Math.abs(o[0]-o[1]) > x).length;
      let p = cnt / total;
      if (p >= 0.10 && p <= 0.90) validX.push({ x, cnt });
    }
    if (!validX.length) return null;
    ...
    return {
      vraag: `Gooi 2 dobbelstenen.\nP(verschil > ${x}) = ?`,
      antwoordStr: fmt(antwoord), antwoord,
      dice: showDice, label: `verschil>${x}`
    };
  }

  // ── 3 DOBBELSTENEN vragen ─────────────────────────────────

  if (type === 7) {
    // P(som = x) met 3 dobbelstenen
    let outcomes3 = allOutcomes(3);
    let total3 = 216;
    let somCounts = {};
    for (let o of outcomes3) {
      let s = o[0]+o[1]+o[2];
      somCounts[s] = (somCounts[s]||0) + 1;
    }
    let candidates = Object.entries(somCounts)
      .map(([s, cnt]) => ({ s: Number(s), cnt }))
      .filter(({ cnt }) => cnt > 0 && cnt < total3);
    if (!candidates.length) return null;
    let { s, cnt } = candidates[Math.floor(Math.random() * candidates.length)];
    let antwoord = fmt2(cnt / total3);
    if (antwoord <= 0 || antwoord >= 1) return null;
    return {
      vraag: `Gooi 3 dobbelstenen.\nP(som = ${s}) = ?`,
      antwoordStr: fmt(antwoord), antwoord,
      dice: [showDice[0], showDice[1], Math.floor(Math.random()*6)+1],
      label: `3d som=${s}`
    };
  }
  ...
  if (type === 8) {
    ...
    let validX = [];
    for (let x = 3; x <= 17; x++) {
      let cnt = outcomes3.filter(o => o[0]+o[1]+o[2] > x).length;
      let p = cnt / total3;
      if (p >= 0.05 && p <= 0.95) validX.push({ x, cnt });
    }
    if (!validX.length) return null;
  ...
  if (type === 9) {
    // P(som < x) met 3 dobbelstenen — type 9 toevoegen
    let outcomes3 = allOutcomes(3);
    let total3 = 216;
    let validX = [];
    for (let x = 4; x <= 18; x++) {
...
function generateQuestions() {
  ...
  let questions   = [];
  let usedAnswers = {}; // antwoord (string) → aantal keer gebruikt
  let usedLabels  = new Set(); // voorkom exacte dubbele vragen
  let attempts    = 0;

  while (questions.length < 10 && attempts < 5000) {
    attempts++;

    let q = tryMakeQuestion();
    if (!q) continue;

    // Voorkom exacte dubbele vraag
    if (usedLabels.has(q.label)) continue;

    // Max 2× hetzelfde antwoord
    if ((usedAnswers[q.antwoordStr] || 0) >= 2) continue;

    usedLabels.add(q.label);
    usedAnswers[q.antwoordStr] = (usedAnswers[q.antwoordStr] || 0) + 1;
    questions.push(q);
  }
  ...
  while (questions.length < 10) {
    questions.push({
      vraag: 'Je gooit 2 dobbelstenen.\nKans dat de som 7 is?',
      antwoordStr: '0,17',
      antwoord: 0.17,
      dice: [3, 4],
      label: 'nood'
  ...
  // Antwoorden shufflen
  let answers = shuffle(questions.map(q => q.antwoordStr));
  ...
  for (let row = 1; row < 3; row++) {
    for (let col = 0; col < COLS; col++) {
      let q = questions[qi++];
      blocks.push({
        col, row, startCol: col, startRow: row,
        x: MARGIN + col * CELL_SIZE,
        y: MARGIN + row * CELL_SIZE + BUTTON_HEIGHT + TITLE_SPACE,
        isDragging: false, isPlaced: false,
        text: q.vraag,
        dice: q.dice,
        answer: q.antwoordStr,
        answerNum: q.antwoord,
        isQuestion: true, isCorrect: null, isHovered: false, hoverProgress: 0,
      });
      qi <= 10 && qi++;
      qi--; // correctie
    }
  ...
  for (let row = 3; row < 5; row++) {
    for (let col = 0; col < COLS; col++) {
      blocks.push({
        ...
        text: answers[ai],
        answer: answers[ai],
        isQuestion: false, isCorrect: null, isHovered: false, hoverProgress: 0,
      });
      ai++;
    }
  }
...
// ── Aangepaste checkAnswers ───────────────────────────────────
// Vergelijkt op string (bv. "0,17") want antwoorden zijn decimale strings
...
function checkAnswers() {
  ...
  for (let qBlock of blocks) {
    if (!qBlock.isQuestion) continue;
    let aBlock = blocks.find(b =>
      !b.isQuestion && b.col === qBlock.col && b.row === qBlock.row
    );
    if (aBlock && qBlock.answer === aBlock.answer) {
      qBlock.isCorrect = true;
      aBlock.isCorrect = true;
      correctCount++;
    } else {
      qBlock.isCorrect = false;
      if (aBlock) aBlock.isCorrect = false;
    }
  }
...
// ── Dobbelsteenafbeelding via Twemoji CDN ─────────────────────
// 🎲 = unicode 1f3b2, geladen als PNG via jsDelivr/twemoji
// Wordt ingeladen in setup() via: diceImg = loadImage(DICE_IMG_URL)

var DICE_IMG_URL = 'https://cdn.jsdelivr.net/gh/jdecked/twemoji@latest/assets/72x72/1f3b2.png';
var diceImg = null;  // wordt gevuld door loadImage in setup()

/**
 * Laad de dobbelsteen afbeelding. Roep dit aan in setup():
 *   diceImg = loadImage(DICE_IMG_URL);
 */
function preloadDiceImage() {
  loadImage(DICE_IMG_URL, function(img) {
    diceImg = img;
  });
}

/**
 * Teken één dobbelsteen emoji op middelpunt (cx, cy) met breedte size.
 * Gebruikt de geladen twemoji afbeelding als beschikbaar,
 * anders fallback naar 🎲 tekst.
 */
function drawDie3D(cx, cy, size, dots) {
  push();
  imageMode(CENTER);
  if (diceImg) {
    image(diceImg, cx, cy, size, size);
  } else {
    // Fallback: emoji tekst
    textAlign(CENTER, CENTER);
    textSize(size * 0.8);
    text('🎲', cx, cy);
  }
...
function drawBlock(block) {
  ...
  stroke(50, 100, 200); strokeWeight(3);
  let bx = (block.isQuestion && block.hoverProgress > 0) ? 5 : block.x + 5;
  let by = (block.isQuestion && block.hoverProgress > 0) ? 5 : block.y + 5;
  rect(bx, by, CELL_SIZE-10, CELL_SIZE-10, 5);
  drawingContext.shadowBlur = 0; drawingContext.shadowOffsetY = 0;

  if (block.isQuestion && block.dice) {
    // ── 3D dobbelstenen bovenin ───────────────────────────
    let n = block.dice.length;
    let dieSize = n === 2 ? 26 : 22;  // grootte per dobbelsteen
    let gap = 6;
    let totalDiceW = n * dieSize + (n-1) * gap;
    // Middelpunt van de hele rij dobbelstenen
    let rowCx = bx + (CELL_SIZE - 10) / 2;
    let dieCy = by + dieSize * 0.85 + 4; // verticaal middelpunt

    for (let i = 0; i < n; i++) {
      // Middelpunt van elke dobbelsteen
      let dieCx = rowCx - totalDiceW/2 + dieSize/2 + i*(dieSize+gap);
      drawDie3D(dieCx, dieCy, dieSize, block.dice[i]);
    }

    // ── Vraagtekst onder de dobbelstenen ─────────────────
    fill(PROB_TEXT_COLOR[0], PROB_TEXT_COLOR[1], PROB_TEXT_COLOR[2]);
    noStroke();
    let pAlign = PROB_TEXT_ALIGN === 'right' ? RIGHT : (PROB_TEXT_ALIGN === 'center' ? CENTER : LEFT);
    textAlign(pAlign, TOP);
    textStyle(PROB_TEXT_STYLE === 'bold' ? BOLD : NORMAL);
    textSize(PROB_TEXT_SIZE);
    textLeading(PROB_TEXT_LINE_H);
    let textY = dieCy + dieSize * 0.85 + PROB_TEXT_OFFSET_Y;
    // X positie: links uitgelijnd = linkermarge, center = midden blokje
    let textX = PROB_TEXT_ALIGN === 'center'
      ? bx + (CELL_SIZE - 10) / 2
      : PROB_TEXT_ALIGN === 'right'
        ? bx + (CELL_SIZE - 10) - PROB_TEXT_MARGIN_X
        : bx + PROB_TEXT_MARGIN_X;
    // Splits handmatig op \n zodat uitlijning correct werkt
    let lines = block.text.split('\n');
    for (let li = 0; li < lines.length; li++) {
      text(lines[li], textX, textY + li * PROB_TEXT_LINE_H);
    }

  } else {
    // Antwoordblok: decimaal getal
    fill(PROB_ANS_TEXT_COLOR[0], PROB_ANS_TEXT_COLOR[1], PROB_ANS_TEXT_COLOR[2]);
    noStroke();
    textAlign(CENTER, CENTER);
    textStyle(BOLD);
    textSize(PROB_ANS_TEXT_SIZE);
    let tx = (block.isQuestion && block.hoverProgress > 0) ? CELL_SIZE/2 : block.x+CELL_SIZE/2;
    let ty = (block.isQuestion && block.hoverProgress > 0) ? CELL_SIZE/2 : block.y+CELL_SIZE/2;
    text(block.text, tx, ty);
  }
  pop();
}
